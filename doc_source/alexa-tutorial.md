...
The main benefits of creating Alexa skills from AWS CodeStar are that you can get started with skill development from within AWS and connect your Amazon developer account to the project to deploy skills to the development stage directly from within AWS\. You also get a ready to use deployment \(CI\) pipeline with a repository with all the source code for the project\. You can configure this repository with your preferred IDE to enable you to create skills with tools you are familiar with\.

## Prerequisites<a name="alexa-prereq"></a>
+  Create an Amazon developer account by going to [https://developer\.amazon\.com](https://developer.amazon.com/)\. Sign up is free\. This account owns your Alexa skills\. 
...
**Important**  
After you create the Alexa skill project, make all edits only in the project repository\. We recommend that you do not edit this skill directly using any other Alexa Skills Kit tools, such as the ASK CLI or ASK developer console\. These tools are not integrated with the project repository, and using them will cause the actual skill and repository code to become out of sync\.

## Step 1: Create the project and connect your Amazon developer account<a name="alexa-tutorial-step1"></a>

In this tutorial, you will create a skill using Node\.js running on AWS Lambda\. Most of the steps are the same for other languages, although the skill name will differ\. Refer to the README\.md file in the project repository for details of the specific project template you choose\.
...
1. For **Project name**, enter a name for the project \(for example, *My Alexa Skill*\)\. If you use a different name from the example, be sure to use it throughout this tutorial\. AWS CodeStar chooses a related identifier for this project for the **Project ID** \(for example, **my\-alexa\-skill**\)\. If you see a different project ID, be sure to use it throughout this tutorial\. 
...
1. Sign in with your Amazon developer credentials\. Click **Allow**\.
...
As part of creating the project, AWS CodeStar also created AWS resources on your behalf\. These resources include a project repository using CodeCommit, a deployment pipeline using CodePipeline and an AWS Lambda function\. You can access these resources from the left navigation bar\. For example, choosing **Code** will open the project CodeCommit repository\. You can view the pipeline deployment status in the **Continuous deployment** tile on the project dashboard \(choose **Dashboard** from the left navigation bar, if necessary\)\. You can view a complete list of AWS resources created as part of your project by choosing **Project** in the left navigation bar\. This list includes links to each resource\.

## Step 4: Make a change in your skill's response<a name="alexa-tutorial-step4"></a>

In this step, you will make a minor change to your skill's response to understand the iteration cycle\.
...
1. Scroll down to the end of the file\. Enter author name and email address as well as an optional commit message\.
...
Previously you made a small change to the source code directly from the CodeCommit console\. In this step you will configure the project repository with your local workstation so that you can edit and manage code from the command line or your favorite IDE\. The following steps explain how to set up command line tools\.
...
1. Follow the displayed instructions to complete the following tasks:

   1. Install Git on your local workstation from a website such as [Git Downloads\.](https://git-scm.com/downloads)

   1. Install the AWS CLI\. For more information, see [Installing the AWS Command Line Interface](http://docs.aws.amazon.com/cli/latest/userguide/installing.html)\.

   1. Configure the AWS CLI with your IAM user access key and secret key\. For more information, see [Configuring the AWS CLI](https://docs.aws.amazon.com/cli/latest/userguide/cli-chap-configure.html)\.

   1. Clone the project's AWS CodeCommit repository onto your local workstation\. For more information, see [Connect to an AWS CodeCommit Repository](https://docs.aws.amazon.com/codecommit/latest/userguide/how-to-connect.html)\.

## Next Steps \- Continue your skill development journey<a name="alexa-tutorial-next"></a>
